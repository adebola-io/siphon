...
pp.parseTopLevel = function (node) {
  ...
  if (this.inModule) 
    for (let name of Object.keys(this.undefinedExports)) this.raiseRecoverable(this.undefinedExports[name].start, `Export '${name}' is not defined`);
...
pp.isLet = function (context) {
  ...
  if (nextCh === 91 || nextCh === 92 || nextCh > Oxd7ff && nextCh < Oxdc00) 
    return true;
  if (context) 
    return false;
  if (nextCh === 123) 
  ...
  if (isIdentifierStart(nextCh, true)) {
    ...
    if (nextCh === 92 || nextCh > Oxd7ff && nextCh < Oxdc00) 
      return true;
...
pp.isAsyncFunction = function () {
  ...
  return !lineBreak.test(this.input.slice(this.pos, next)) && this.input.slice(next, next + 8) === "function" && (next + 8 === this.input.length || !isIdentifierChar(after = this.input.charCodeAt(next + 8)) || after > Oxd7ff && after < Oxdc00);
};
...
pp.parseBreakContinueStatement = function (node, keyword) {
  ...
  return this.finishNode(node, isBreak 
    ? "BreakStatement"
    : "ContinueStatement");
};
...
pp.parseForStatement = function (node) {
  this.next();
  let awaitAt = this.options.ecmaVersion >= 9 && this.canAwait && this.eatContextual("await") 
    ? this.lastTokStart
    : -1;
  this.labels.push(loopLabel);
  this.enterScope(0);
  ...
  if (this.type === tt._var || this.type === tt._const || isLet) {
    let init = this.startNode(), kind = isLet 
      ? "let"
      : this.value;
    this.next();
  ...
  let init = this.parseExpression(awaitAt > -1 
    ? "await"
...
pp.parseFunctionStatement = function (node, isAsync, declarationPosition) {
  ...
  return this.parseFunction(
    node, 
    FUNC_STATEMENT | declarationPosition 
      ? 0
...
pp.parseTryStatement = function (node) {
  ...
  if (this.type === tt._catch) {
    ...
    if (this.eat(tt.parenL)) {
      ...
      this.enterScope(simple 
        ? SCOPE_SIMPLE_CATCH
        : 0);
      this.checkLValPattern(clause.param, simple 
        ? BIND_SIMPLE_CATCH
        : BIND_LEXICAL);
...
pp.parseLabeledStatement = function (
  node, 
  maybeName, 
  expr, 
  context
) {
  for (let label of this.labels) if (label.name === maybeName) 
...
pp.parseForIn = function (node, init) {
  ...
  if (init.type === "VariableDeclaration" && init.declarations[0].init != null && !isForIn || this.options.ecmaVersion < 8 || this.strict || init.kind !== "var" || init.declarations[0].id.type !== "Identifier") {
    this.raise(init.start, ` loop variable declaration may not have an initializer${isForIn 
      ? "for-in"
      : "for-of"}`);
  }
  node.left = init;
  ...
  this.exitScope();
  this.labels.pop();
  return this.finishNode(node, isForIn 
    ? "ForInStatement"
    : "ForOfStatement");
};
...
pp.parseVarId = function (decl, kind) {
  ...
  this.checkLValPattern(decl.id, kind === "var" 
    ? BIND_VAR
    : BIND_LEXICAL, false);
};
const FUNC_STATEMENT = 1, FUNC_HANGING_STATEMENT = 2, FUNC_NULLABLE_ID = 4;
pp.parseFunction = function (
  node, 
  statement, 
  allowExpressionBody, 
  isAsync, 
  ...
  if (statement & FUNC_STATEMENT) {
    node.id = statement & FUNC_NULLABLE_ID && this.type !== tt.name 
      ? null
      : this.parseIdent();
  ...
  return this.finishNode(node, statement & FUNC_STATEMENT 
    ? "FunctionDeclaration"
    : "FunctionExpression");
};
