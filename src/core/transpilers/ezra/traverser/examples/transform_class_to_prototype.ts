 ...
 */
function transform_class_to_prototype(node: ClassDeclaration) {
  ...
  node.body.body.forEach((def) => {
    if (
      ...
      let dot = memberExpression(this_, def.key);
      dot.computed = def.computed || !(def.key instanceof Identifier);
      let assign = assignmentExpression(dot, "=", def.value);
      ...
      switch (def.kind) {
        case "constructor":
          ...
          break;
        case "method":
          let mem1 = memberExpression(node.id, newIdentifier("prototype"));
          let mem2 = def.static
            ? memberExpression(node.id, def.key)
            : memberExpression(mem1, def.key);
          mem2.computed = def.computed || !(def.key instanceof Identifier);
          let assign = assignmentExpression(mem2, "=", def.value);
          let exp = new ExpressionStatement(0);
          exp.expression = assign;
          methods.push(exp);
          break;
        ...
        case "set":
          ...
          let getcall = callExpression(ob, [
            def.static ? node.id : memberExpression(node.id, prototype),
            key,
            object,
          ]);
      ...
      let ob = memberExpression(node.id, def.key);
      ob.computed = def.computed || !(def.key instanceof Identifier);
      let assign = assignmentExpression(ob, "=", def.value);
      let exp = new ExpressionStatement(0);
      exp.expression = assign;
      methods.push(exp);
    }
