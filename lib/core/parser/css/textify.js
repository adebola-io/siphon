 ...
 */
function textify(source, assets) {
    var foreignImports = [];
    var weirdImports = [];
        ...
        for (var i = 0; cssText[i]; i++) {
            ...
            if (cssText.slice(i, i + 2) === "/*") {
                ...
                if (store.includes("https://") || store.includes("http://")) {
                    ...
                    else if (assets[path.basename(truePath)] &&
                        assets[path.basename(truePath)] === truePath) {
                        text += "\"./".concat(path.basename(truePath), "\")");
                    }
                    else {
                        var a = 1;
                        while (assets[(0, getFileName_1.default)(truePath) + "-" + a + path.extname(truePath)]) {
                            a++;
                        }
                        var newname = (0, getFileName_1.default)(truePath) + "-" + a + path.extname(truePath);
                        weirdImports.push({
                            srcpath: truePath,
                            name: newname,
                        });
